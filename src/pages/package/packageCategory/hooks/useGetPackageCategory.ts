...
import type { PackageCategoryListItemResponse } from '../interface/IPackageCategory';
import useDisclosure from '@/utils/useDisclosure';
import { useState } from 'react';
import type { RowSelectionState } from '@tanstack/react-table';
import { useDebounce } from '@/utils/useDebounce';

export const useGetPackageCategory = () => {
  ...
  const { data, isError, isLoading, isSuccess } = useGetDataQuery<{
    data: PackageCategoryListItemResponse;
    isLoading: boolean;
    isError: boolean;
    isSuccess: boolean;
  }>({
    ...
    tag: apiTags.packages.packageCategory.list,
  });
