...
import type { PackageGalleryListItemResponse } from '../interface/IPackageGallery';
import useDisclosure from '@/utils/useDisclosure';
import { useState } from 'react';
import type { RowSelectionState } from '@tanstack/react-table';
import { useDebounce } from '@/utils/useDebounce';

export const useGetPackageGallery = () => {
  ...
  const { data, isError, isLoading, isSuccess } = useGetDataQuery<{
    data: PackageGalleryListItemResponse;
    isLoading: boolean;
    isError: boolean;
    isSuccess: boolean;
  }>({
    ...
    tag: apiTags.packages.packageGallery.list,
  });

  return {
    packageGalleryData: data,
    isError,
    isLoading,
    isSuccess,
    createModal,
