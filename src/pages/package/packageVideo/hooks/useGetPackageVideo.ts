...
import type { PackageVideoListItemResponse } from '../interface/IPackageVideo';
import useDisclosure from '@/utils/useDisclosure';
import { useState } from 'react';
import type { RowSelectionState } from '@tanstack/react-table';
import { useDebounce } from '@/utils/useDebounce';

export const useGetPackageVideo = () => {
  ...
  const { data, isError, isLoading, isSuccess } = useGetDataQuery<{
    data: PackageVideoListItemResponse;
    isLoading: boolean;
    isError: boolean;
    isSuccess: boolean;
  }>({
    ...
    tag: apiTags.packages.packageVideo.list,
  });

  return {
    packageVideoData: data,
    isError,
    isLoading,
    isSuccess,
    createModal,
