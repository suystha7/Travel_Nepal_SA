...
export const useDeletePackageVideo = () => {
  ...
  const handleDelete = async () => {
    const response = (await deleteData({
      url: Endpoints.packages.packageVideo.delete.replace(':id', deleteIdState.values),
      invalidates: [apiTags.packages.packageVideo.list],
    })) as ApiResponse;
