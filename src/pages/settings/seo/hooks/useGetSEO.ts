...
import type { StaticSEOListItemResponse } from '../interface/IStaticSEO';
...
export const useGetSEO = () => {
  ...
  const { data, isError, isLoading, isSuccess } = useGetDataQuery<{
    data: StaticSEOListItemResponse;
    isLoading: boolean;
    isError: boolean;
    isSuccess: boolean;
  }>({
    url: Endpoints?.settings.seo.list,
    params: { p: page, page_size: pageSize, search: debouncedSearch },
    tag: apiTags.settings.seo.list,
  });
