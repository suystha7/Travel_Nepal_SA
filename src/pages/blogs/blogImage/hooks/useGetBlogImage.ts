...
import type { BlogImageListItemResponse } from '../interface/IBlogImage';
import useDisclosure from '@/utils/useDisclosure';
import { useState } from 'react';
import type { RowSelectionState } from '@tanstack/react-table';
import { useDebounce } from '@/utils/useDebounce';

export const useGetBlogImage = () => {
  ...
  const { data, isError, isLoading, isSuccess } = useGetDataQuery<{
    data: BlogImageListItemResponse;
    isLoading: boolean;
    isError: boolean;
    isSuccess: boolean;
  }>({
    ...
    tag: apiTags.blogs.blogImage.list,
  });
